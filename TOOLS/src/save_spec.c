...
int	save_spec( obs_name, stn_name, stn_id, obj_name, ssnum, freq_num_ptr,
	...
	double	**vis_i_ptr;			/* Pointer of Visibility Data (imag) */
{
	...
	/*-------- OPEN BANDPASS DATA --------*/
	sprintf(fname, "CALIB/%s.%s.%s.%s.%12.6lf",
			obs_name, stn_name, obj_name, ant_stat[scan_type], start_mjd); 
