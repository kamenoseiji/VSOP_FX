...
int	cpg_bp( obs_name, stn_name, obj_name,	start_mjd, stop_mjd, integ_time,
			ssnum, freq_num_ptr, rf_ptr, freq_incr_ptr,
			vis_max_ptr, vis_r_ptr, vis_i_ptr )
	...
	double	**vis_i_ptr;			/* Pointer of Visibility Data */
{
	...
	char	text[32];
	char	pg_device[32];
	double	v_min, v_max, v_sqr;
	double	sum_x, sum_xx, sum_y, sum_xy, sum_yy, resid;
	double	afact0, afact1;
...
#ifdef HIDOI
		sum_x	= 0.0;	sum_xx = 0.0; sum_y = 0.0; sum_xy = 0.0;
		for( freq_index=32; freq_index<252; freq_index++){
			sum_x	+= (double)freq_index;
			sum_xx	+= (double)(freq_index * freq_index);
			sum_y	+= vis_amp_ptr[freq_index];
			sum_xy	+= (vis_amp_ptr[freq_index] * (double)freq_index);
			sum_yy	+= (vis_amp_ptr[freq_index] * vis_amp_ptr[freq_index]);
		}
		afact1	= (220.0*sum_xy - sum_x*sum_y)/(220.0*sum_xx - sum_x*sum_x);
		afact0	= (sum_xx*sum_y  - sum_x*sum_xy)/(220.0*sum_xx - sum_x*sum_x);

		v_sqr = 0.0; v_min = 1.0e30; v_max = -1.0e30;
		for( freq_index=32; freq_index<252; freq_index++){
			resid	= vis_amp_ptr[freq_index]
					- afact1*(double)freq_index
					- afact0;
			v_sqr	+= (resid * resid); 

			if( vis_amp_ptr[freq_index] > v_max ){
				v_max = vis_amp_ptr[freq_index]; 
			}
			if( vis_amp_ptr[freq_index] < v_min ){
				v_min = vis_amp_ptr[freq_index]; 
			}
		}
		sum_y	/= 220.0;
		v_sqr	= sqrt(v_sqr / 220.0);
		v_sqr	= sqrt(sum_yy / 220.0 - sum_y*sum_y);

		printf("%s : MAX = %10.3lf MIN=%10.3lf AVE=%10.3lf SD=%10.3lf\n",
			stn_name, v_max, v_min, sum_y, v_sqr);
#endif
		...
		for( freq_index=0; freq_index<*freq_num_ptr; freq_index++){
			*freq_ptr = xmin + freq_index * (*freq_incr_ptr);
			...
			if(*vis_amp_ptr > ymax*0.001){
				...
				#ifdef DEBUG
				printf("STN= %s FREQ= %8.5f  AMP= %10.5f  PHS= %8.5f \n",
					stn_name, *freq_ptr, *vis_amp_ptr, *vis_phs_ptr );
				#endif
