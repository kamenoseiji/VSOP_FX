...
long	gff(vr_ptr, vi_ptr,	spec_num, spec_dim, freq_init, freq_incr,
			time_num, time_dim, time_incr, ant_num, bl_weight_ptr,
			vis_amp_ptr, vis_phs_ptr, ant_delay_ptr, ant_rate_ptr)
...
*/
	...
	afact_num		= (ant_num + 2)*(ant_num - 1);	/* Number of Variable */
	init_vr_ptr	= vr_ptr;
	init_vi_ptr	= vi_ptr;
...
*/
	...
	while(1){
		...
		for(itime=0; itime<time_num; itime++){
			...
			time	= time_incr*(itime - time_num/2);	/* TIME in SEC */
			...
			for(ispec=0; ispec<spec_num; ispec++){
				...
				d_omega	= PI2 * freq_incr * (ispec - spec_num/2);
		...
		/*-------- CONVERGED OR NOT --------*/
		if( (cor < 1.0e-4) || (niter > MAX_ITER) ){ break;}
		niter++;
	}

	/*-------- OUTPUT VALUE (baseline-based) --------*/
	for(i=0; i<baseline_num; i++){
