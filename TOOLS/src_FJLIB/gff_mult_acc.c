/*********************************************************
**	GFF_MULT_ACC.C	: GLOBAL FRINGE FITTING module for	**
...
long	gff_mult_acc(vr_ptr_ptr,	vi_ptr_ptr,		ss_num,		spec_num_ptr,
	...
	float	*afact_err_ptr;	/* OUTPUT: Pointer of Error of Parameters		*/
{
	...
	long	acc_index;			/* Delay ACC Index in afact[] */
	...
	double	rel_time;			/* Time from the Reference [sec] */
			...
			for(ss_index=0; ss_index<ss_num; ss_index++){
				...
				for(time_index=0; time_index<time_num; time_index++){
					...
					for(spec_index=0; spec_index < *spec_num_ptr; spec_index++){
						...
						if(ant2 == ant_num - 1){
							pr[acc_index]	=-0.5* rel_time* rel_time* trial_vi;
							pi[acc_index]	= 0.5* rel_time* rel_time* trial_vr;
						} else {
							pr[acc_index]	= 0.0;
							pi[acc_index]	= 0.0;
						}
