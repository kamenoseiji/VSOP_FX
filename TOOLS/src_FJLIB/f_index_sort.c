/*************************************************************
**	F_INDEX_SORT.C : Make Index Table of FLOAT Variables	**
**					in Increasing Order.					**
**															**
**	AUTHOR : Kameno S.										**
**	CREATED: 1997/2/27										**
...
int	f_index_sort( data_num, data_ptr, index_ptr )
	int		data_num;				/* Total Number of Data		*/
	float	*data_ptr;				/* Total Number of Data		*/
	int		*index_ptr;				/* Pointer of Index			*/
{
	int		seq_index;				/* Number of Data in Branch	*/
	int		level_index;
	int		i, j;
	float	*temp_ptr;				/* Temporary Data Pointer	*/
	float	curr_top_value;			/* Current Top (smallest)	*/
	int		curr_index;				/* Current Index Value		*/

	/*-------- Copy Data into Temporary Buffer --------*/
	temp_ptr = (float *)malloc( data_num * sizeof(float));
	memcpy( temp_ptr, data_ptr, data_num * sizeof(float) );
