/*********************************************************
**	VANVLECK.C :Conduct Van-Vleck Correction for 1-bit	**
**		Quantized Data									**
**														**
**	AUTHOR	: KAMENO Seiji								**
**	CREATED	: 1996/6/27									**
...
double	vanvleck( freq_num, power, re_ptr, im_ptr )
...
/*
		printf("KAMENO : VIS_R[%d] = %lf\n", freq_index, re_ptr[freq_index]);
*/
		vis_r_ptr[freq_index]	= re_ptr[freq_index];
	}

	/*-------- NULL PADDING INTO IMAG DATA for AUTOCORR --------*/
	if(power <= 0.0){
		for(freq_index=0; freq_index<freq_num; freq_index++){
			vis_i_ptr[freq_index]	= 0.0;
		}
	} else {
		for(freq_index=0; freq_index<freq_num; freq_index++){
	...
	for(freq_index=0; freq_index<nfft; freq_index++){
		...
		vis_i_ptr[freq_index]	= sin(afact * vis_i_ptr[freq_index]);
	}
...
*/
	...
	free(vis_r_ptr);
	free(vis_i_ptr);
	printf("KAMENO : POWER[IN]=%lf  POWER[OUT]=%lf\n", power, M_PI_2/afact);
