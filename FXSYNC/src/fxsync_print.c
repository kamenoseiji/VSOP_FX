/*********************************************************
**	FXSYNC_PRINT.C : Generate PS File of FXSYNC STATUS	**
**														**
	...
	struct shared_mem2 *fxsched_ptr;	/* Pointer of Shared Memory	*/
{
	...
	cpgsci(UNDEF);	cpgrect(xmin, xmax, ymin, ymax);
	...
	struct shared_mem2 *fxsched_ptr;	/* Pointer of Shared Memory	*/
{
	...
	if( fxsync_ptr->utcset_done != 0 ){
		...
		cpgdraw(x_text, ymax);
		cpgsch(1.0);
		y_text = -0.1; cpgpt(1, &x_text, &y_text, -4);
		y_text = 10.1; cpgpt(1, &x_text, &y_text, -4);
	}
	cpgsch(0.7);
	...
	struct shared_mem2 *fxsched_ptr;	/* Pointer of Shared Memory	*/
{
	...
	for( dms_index=0; dms_index<5; dms_index++){
		...
		switch( cart_stat ){
			case UNDEF		: coltable = UNDEF;			break;
