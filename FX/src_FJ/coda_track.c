/*********************************************************
**	CODA_TRACK.C	: Integrate Visibility in CFS and Disp	**
	...
	char	**argv;		/* Pointer of Arguments */
{
	...
	int		nx_index, ny_index;			/* Window Index						*/
	...
	int		ppnum_bunch[MAX_BLOCK];		/* PP Number in Each PP Block		*/
	...
	int		err_code;					/* PGPLOT Error Code				*/
	double	*time_data_ptr;				/* Pointer of Time Data				*/
	float	xmin, xmax;					/* Plot Range						*/
	float	amp_min, amp_max;			/* Plot Range						*/
	float	phs_min, phs_max;			/* Plot Range						*/
	float	xwin_incr, ywin_incr;		/* Window Position Increment		*/
	char	text[64];					/* PGPLOT Text						*/
...
*/
	if( argc < 10 ){
		printf("USAGE : coda_track [OBS_NAME] [SOURCE] [DELAY CAL] [START] [STOP] [INTEG] [BUNCH] [DEVICE] [AMP] [STN_NAME1] [STN_NAME2] [TOTAL_SS] [SS1] [SS2] ... !!\n");
