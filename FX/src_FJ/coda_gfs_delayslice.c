...
#define	VISIB_FMT	" BL%2d : SS=%2d: AMP[%%]= %7.5lf +/- %8.6lf, PHS[deg]= %7.2lf +/- %7.3lf at U,V = %8.2f, %8.2f\n"
	...
	for( bl_index=0; bl_index<blnum; bl_index++){
		...
		*bl_rate_ptr	= -coeff[3]/(coeff[1]
		...
		vis_snr	= vismax*sqrt(2.0*time_num*time_incr
				*freq_num[0]*freq_incr[0]*1.0e6*sqrt(ssnum));
...
#ifdef PGSLICE
#endif
		cpgdelayslice( visamp_ptr, wind_x, wind_x, x_incr,	/* DELAY Slice */
