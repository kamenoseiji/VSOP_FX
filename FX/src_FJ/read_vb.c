/*********************************************************
**	READ_VB.C :	Read Variable-Binary Data in CFS.	 	**
**														**
**	FUNCTON	: Read Valiable-Binary Data in CFS and		**
**				Load to Buffer Memory. This module does	**
...
int	read_vb( fname,	mjd_ptr_ptr, size_ptr_ptr, buf_ptr_ptr)
	char	*fname;					/* VB File Name		 			*/
	int		*mjd_ptr_ptr;			/* Pointer of MJD				*/
	int		*size_ptr_ptr;			/* Pointer of Data Size			*/
	int		*buf_ptr_ptr;			/* First Pointer of Buffer data */
{
	...
	int		vbdata_num;		/* Number of Delay Data					*/
	int		*buf_ptr;		/* Pointer of Buffer					*/
	int		*size_ptr;		/* Pointer of Data Size					*/
	double	mjd;			/* MJD in Data							*/
	double	*mjd_ptr;		/* Pointer of MJD in CFS				*/
	...
	size_ptr= (int *)malloc( vbdata_num * sizeof(int) );
	...
	while( vb_ptr != NULL ){
		...
		size_ptr[data_index]= vb_ptr->data_size;
