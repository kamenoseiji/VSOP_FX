/*********************************************************
**	CODA_AMPHI.C	: Global Fringe Search using 		**
**					CODA File System					**
**														**
**	AUTHOR	: KAMENO Seiji								**
**	CREATED	: 1996/6/27									**
	...
	char	**envp)			/* Pointer to Environments		*/
{
	...
	int		vis_amp[MAX_BL*MAX_SS];		/* Pointer of VisAmp vs Time		*/
	int		vis_phs[MAX_BL*MAX_SS];		/* Pointer of VisPhs vs Time		*/
	int		err_amp[MAX_BL*MAX_SS];		/* Pointer of VisAmpErr vs Time		*/
	int		err_phs[MAX_BL*MAX_SS];		/* Pointer of VisPhsErr vs Time		*/
	int		cl_phs[MAX_CL*MAX_SS];		/* Pointer of VisPhsErr vs Time		*/
	int		cl_err[MAX_CL*MAX_SS];		/* Pointer of VisPhsErr vs Time		*/
	...
	int		ssnum_in_cfs;				/* Total Number of SS in CFS		*/
	...
	int		node_gain_num[MAX_STN];		/* Total Number of Rate Node		*/
...
*/
	if( argc < 9 ){
		printf("USAGE : coda_avail [OBS_NAME] [SOURCE] [START] [STOP] [INTEG] [TOTAL STN] [TOTAL SS] [STN_NAME1] [STN_NAME2] ...  [SS1] [SS2] ...!!\n");
...
*/
	...
	for(bl_index=0; bl_index<bl_num; bl_index++ ){
		for(ss_index=0; ss_index<ss_num; ss_index++){
			vis_amp[bl_index*ss_num+ss_index] = malloc(time_num* sizeof(float));
			vis_phs[bl_index*ss_num+ss_index] = malloc(time_num* sizeof(float));
			err_amp[bl_index*ss_num+ss_index] = malloc(time_num* sizeof(float));
			err_phs[bl_index*ss_num+ss_index] = malloc(time_num* sizeof(float));
		}
	}
	for(cl_index=0; cl_index<cl_num; cl_index++ ){
		for(ss_index=0; ss_index<ss_num; ss_index++){
			cl_phs[cl_index*ss_num+ss_index] = malloc(time_num* sizeof(float));
			cl_err[cl_index*ss_num+ss_index] = malloc(time_num* sizeof(float));
		}
	}
	...
	for(bl_index=0; bl_index<bl_num; bl_index++ ){
		...
		for(ss_index=0; ss_index<ss_num; ss_index++){
			...
			ave_bp_r[bl_index][ss_index]
					= malloc(bp_freq_num[ss_index]* sizeof(double) );
			ave_bp_i[bl_index][ss_index]
					= malloc(bp_freq_num[ss_index]* sizeof(double) );
			ave_bandpass( bp_freq_num[ss_index],
				bp_r_ptr[ant1][cfs_ssid[ss_index] - 1],
				bp_i_ptr[ant1][cfs_ssid[ss_index] - 1],
				bp_r_ptr[ant2][cfs_ssid[ss_index] - 1],
				bp_i_ptr[ant2][cfs_ssid[ss_index] - 1],
		        ave_bp_r[bl_index][ss_index], ave_bp_i[bl_index][ss_index] );
			bp_power[bl_index][ss_index] = vanvleck2( bp_freq_num[ss_index],
	...
	while( (curr_mjd + 0.5*integ/SECDAY) < stop_mjd ){
		...
		for(bl_index=0; bl_index<bl_num; bl_index++ ){
			...
			for(ss_index=0; ss_index<ss_num; ss_index++){
				...
				/*-------- RELATION BETWEEN INPUT SS and CFS SS LIST --------*/
				icon = read_sshead( cfs_blid[bl_index], cfs_ssid[ss_index],
				...
				average_vis(
					...
					ave_bp_r[bl_index][ss_index],
					ave_bp_i[bl_index][ss_index],
					bp_power[bl_index][ss_index],
					cfs_blid[bl_index], cfs_ssid[ss_index],
					bldelay, blrate, blsefd,
					rf[ss_index], rf[ss_index], freq_incr[ss_index], freq_num[ss_index],
					curr_mjd, integ, time_incr, time_num_cfs,
					&position, &first_mjd, uvw, work );

				vis_ptr = (float *)vis_amp[bl_index*ss_num+ss_index];
				vis_ptr[time_index] = ave_vis[(bl_index* ss_num+ ss_index)* 2];

				vis_ptr = (float *)vis_phs[bl_index*ss_num+ss_index];
				vis_ptr[time_index] = ave_vis[(bl_index* ss_num+ ss_index)*2+1];

				vis_ptr = (float *)err_amp[bl_index*ss_num+ss_index];
				vis_ptr[time_index] = err_vis[(bl_index* ss_num+ ss_index)* 2];

				vis_ptr = (float *)err_phs[bl_index*ss_num+ss_index];
				vis_ptr[time_index] = err_vis[(bl_index* ss_num+ ss_index)*2+1];
		...
		for(cl_index=0; cl_index<cl_num; cl_index++ ){
			...
			for(ss_index=0; ss_index<ss_num; ss_index++){
				vis_ptr = (float *)cl_phs[cl_index*ss_num+ss_index];
				vis_ptr[time_index] = (float)remainder( (
					ave_vis[(bl1* ss_num + ss_index)* 2 + 1]
				  -	ave_vis[(bl2* ss_num + ss_index)* 2 + 1]
				  +	ave_vis[(bl3* ss_num + ss_index)* 2 + 1]), 2.0*M_PI );

				vis_ptr = (float *)cl_err[cl_index*ss_num+ss_index];
				vis_ptr[time_index] = sqrt(
					err_vis[(bl1* ss_num + ss_index)* 2 + 1] *
					err_vis[(bl1* ss_num + ss_index)* 2 + 1]
				  +	err_vis[(bl2* ss_num + ss_index)* 2 + 1] *
					err_vis[(bl2* ss_num + ss_index)* 2 + 1]
				  +	err_vis[(bl3* ss_num + ss_index)* 2 + 1] *
					err_vis[(bl3* ss_num + ss_index)* 2 + 1] );
			}
		}
...
/*
	for(time_index=0; time_index<time_num; time_index++){
		vis_ptr = (float *)vis_amp[0];
		printf("%f : %6.4f ", time_list[time_index], 100.0*vis_ptr[time_index]);
		vis_ptr = (float *)err_amp[0];
		printf("+/- %6.4f \n", 100.0*vis_ptr[time_index]);
	}
*/
	...
	for(bl_index=0; bl_index<bl_num; bl_index++){
		for(ss_index=0; ss_index<ss_num; ss_index++){
			free(ave_bp_r[bl_index][ss_index]);
			free(ave_bp_i[bl_index][ss_index]);
		}
	}
