...
*/
	...
	afact_num	= baseline_num*(ss_num + 1)
				+ 2*(ant_num - 1);					/* Number of Variable */
...
*/
	...
	while(niter < MAX_ITER){
			...
			for(time_index=0; time_index<time_num; time_index++){
				...
				for(spec_index=0; spec_index < *spec_num_ptr; spec_index++){
					...
					for(bl_index=0; bl_index < baseline_num; bl_index++){
						...
						/*-------- INDEX in AFACT --------*/
						amp_index	= bl_index;
						phs_index	= baseline_num*(ss_index + 1) + bl_index;
						delay_index1=  baseline_num*(ss_num + 1) + ant1 - 1;
						delay_index2=  baseline_num*(ss_num + 1) + ant2 - 1;
		...
		printf(" AMP= %8.2e %8.2e %8.2e\n", afact[0], afact[1], afact[2]);
		cor = 0.0;
		for(i=0; i<afact_num; i++){
			afact[i]+= d[i];
			cor		+= d[i]*d[i];
		...
		/*-------- CONVERGED ? --------*/
		printf("ITERATION %2d: CHI^2 = %6.3f \n", niter, residual);
