/*********************************************************************
**	BUNCHSEL_VISP.C	: Load Visibility Phase with P-Cal Correction	**
**																	**
**	AUTHOR	: KAMENO Seiji											**
**	CREATED	: 1996/6/27												**
*********************************************************************/
...
int	bunchsel_visp( bl_index, bl_direction, obj_id, ss_index, position,
	...
	double	bl_delay;				/* Baseline-Based Delay [microsec]	*/
	double	bl_rate;				/* Baseline-Based Rate [microsec/sec]*/
		...
		if(		(current_obj ==  obj_id)
			...
			for(freq_index=0; freq_index<*freq_num_ptr/bunch_num; freq_index++){
				...
				for(bunch_index=0; bunch_index<bunch_num; bunch_index++){
					...
					omega	= PI2* (rf + freq_incr*
							(freq_index + bunch_index + (*freq_num_ptr)/2) );

					d_omega	= PI2* freq_incr*
							(freq_index + bunch_index + (*freq_num_ptr)/2);

					phase	= pcal_phs2 - pcal_phs1
							+ d_omega*delay + omega*rate*time;
