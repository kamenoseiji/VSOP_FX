...
/*
#define		VISAMP_FN	 *vis_r
*/
#define		VISAMP_FN	 sqrt( (*vis_r)*(*vis_r) + (*vis_i)*(*vis_i) )

long	crs_search(vr_ptr, vi_ptr,	freq_num,
...
*/
	...
	for(time_index=0; time_index<time_num; time_index++){
		for(freq_index=0; freq_index<freq_num; freq_index++){
			*vis_r	= *vr_ptr;	vr_ptr++; vis_r++;
			*vis_i	= *vi_ptr;	vi_ptr++; vis_i++;
		}
