...
int	cpg_spec( stn_name1, stn_name2, obs_name, obj_name,
...
*/
	...
	center_mjd	= 0.5* (time_data_ptr[0] + time_data_ptr[time_num - 1] );
	fmjd2doy( center_mjd,
...
*/
	...
	for(ss_index=0; ss_index<ssnum; ss_index++){
		for(freq_index=0; freq_index< freq_num_ptr[ss_index]; freq_index++){
			*visr_ave[ss_index] = *visr_sum[ss_index] / time_num;
			*visi_ave[ss_index] = *visi_sum[ss_index] / time_num;
			visr_sum[ss_index] ++;	visi_sum[ss_index] ++;
...
*/
	for(ss_index=0; ss_index<ssnum; ss_index++){
		...
		/*-------- (REAL, IMAG) -> (AMP, PHASE) --------*/
		bldelay = 0.0; blrate  = 0.0;
		cp_vis( rf_ptr[ss_index], freq_incr_ptr[ss_index],
			integ_time* (time_index - time_num/2), bldelay, blrate,
	...
	for(ss_index=0; ss_index<ssnum; ss_index++){
		...
		visr_sum[ss_index] -= freq_num_ptr[ss_index];
		visi_sum[ss_index] -= freq_num_ptr[ss_index];
		fwrite(visr_sum[ss_index], 1,
			freq_num_ptr[ss_index]* sizeof(double), vis_file_ptr);
		fwrite(visi_sum[ss_index], 1,
			freq_num_ptr[ss_index]* sizeof(double), vis_file_ptr);
	}
	fclose(vis_file_ptr);
